...
void MS::Scale(float scale) {
	Mtx44 mat;
	mat.SetToScale(scale, scale, scale);
	ms.top() = ms.top() * mat;
}
...
void MS::Ortho(double left, double right, double bottom, double top, double nearVal, double farVal) {
	Mtx44 mat;
	mat.SetToOrtho(left, right, bottom, top, nearVal, farVal);
	ms.top() = ms.top() *  mat;
}
